...
std::vector<std::vector<Trajectory>> getItemMatrixnonpara(Game* game, const int mask){
...
std::string clonenonparaSolver(SolverParam param, Game* game, SolverIterCallback iter_callback) {
  int num_wrappers = 1;
...
//    cout << epoch << ": ";
    ...
    if(clone_mode && game->num_boosters[BoosterType::CLONING] > 0){
      ...
      cmat = getItemMatrixnonpara(game,  CellType::kSpawnPointBit);
    }else if(!clone_mode && cmat[0].size() == 0 && clone_exist){
      //cout<<"route gen for clone"<<endl;
      clone_mode = true;
      cmat = getItemMatrixnonpara(game, CellType::kBoosterCloningBit);
    }
...
REGISTER_SOLVER("clone_nonpara", clonenonparaSolver);
