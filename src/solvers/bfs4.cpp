...
std::string bfs4Solver(SolverParam param, Game* game, SolverIterCallback iter_callback) {
  int num_add_manipulators = 0;
  while (true) {
    Wrapper* w = game->wrappers[0].get();
    if (game->num_boosters[BoosterType::MANIPULATOR] > 0) {
      w->addManipulator(Point(2 + num_add_manipulators, 0));
      game->tick();
...
REGISTER_SOLVER("bfs4", bfs4Solver);
